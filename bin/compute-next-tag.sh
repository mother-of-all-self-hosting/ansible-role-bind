...
#
# Tags look like `v<BIND version>-<release>`:
#
# - if defaults/main.yml points at a BIND version that has never been
#   released, the release counter restarts at 0 (`v9.20-0`)
# - otherwise the counter is incremented (`v9.20-1`), but only if something
...
# Deliberately keyed on `bind_version`, which is the literal value Renovate
# bumps. Other variables in this file are derived from it through Jinja
# (`bind_container_image_tag: "{{ bind_version }}"`), and reading one of those
# would yield a template rather than a version.
version="$(sed -nE 's|^bind_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the BIND version from $defaults_path"
	exit 1
fi

# The version value does not carry a leading `v` (e.g. `9.20`), while the tags
# do. The stripping keeps this correct either way.
